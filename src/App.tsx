...
function App() {
  const addMessageRef = useRef<((prompt: string, response: string, caseStudy?: LogoData) => void) | null>(null);
  const aiChatRef = useRef<HTMLDivElement>(null);

  const handleLogoClick = (prompt: string, response: string, caseStudy?: LogoData) => {
    if (addMessageRef.current) {
      addMessageRef.current(prompt, response, caseStudy);
    }
  };

  const handleAIChatReady = (addMessage: (prompt: string, response: string, caseStudy?: LogoData) => void) => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center relative">
      ...
      <div className="container max-w-6xl mx-auto z-10 px-4 py-10 mt-24 mb-10">
        ...
        {/* AI Chat Section */}
        <div ref={aiChatRef}>
          <AIChat className="mb-0" onReady={handleAIChatReady} />
        </div>
